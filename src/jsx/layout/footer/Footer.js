...
function Footer(props) {
    ...
    return (
        <footer id='Footer'>
            ...
            <ul className="d-flex flex-wrap justify-content-center align-items-center">
                {
                    icon_list.map(v => {
                        return (
                            <li key={v.id} className="m-3 mx-4">
                                <Link to={v.link}>
                                    {v.icon}
                                </Link>
                            </li>
            ...
            <div className="footer_light_text bg-white p-3 text-center">
                <p className='mb-0'>
                    GREENOVATION WERKSTRASSE 3 3428 WILER
                </p>
                <p className="font_8 mb-0" style={{ fontSize: '12px', lineHeight: '1.8em', textAlign: 'center' }}><span style={{ fontSize: '12px' }}><span className="color_13"><span style={{ letterSpacing: '0.1em' }}>© 2022 Greenovation GmbH. Website by <a href="http://www.brand-brain.ch" target="_blank" rel="noreferrer noopener"><span style={{ fontWeight: 'bold' }}>BRAND</span>BRAIN</a></span></span></span></p>
            </div>
        </footer>
    );
}
